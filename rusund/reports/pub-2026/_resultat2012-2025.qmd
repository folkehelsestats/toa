...
drkAlle <- proscat_weighted("year", "alkosistaar", d = ddt, weight = "vekt", total = FALSE)
drk <- proscat_weighted("year", "alkosistaar", z = "kjonn", d = ddt, weight = "vekt",total = FALSE)
...
make_hist(d = drkDT,
          ...
          group = kjonn2,
          n = count,
          title = "Alkoholbruk siste 12 måneder 2012-2025",
...
drukkMen <- proscat_weighted("year", "alkosistaar", d = ddt[kjonn == 1], weight = "vekt", total = F, z = "agecat")
drukkWom <- proscat_weighted("year", "alkosistaar", d = ddt[kjonn == 2], weight = "vekt", total = F, z = "agecat")
```
...
drikkfreq <- proscat_weighted("year", "drukket2rec", d = ddt, weight = "vekt", total = F)
drikkfreqKjonn <- proscat_weighted("year", "drukket2rec", z="kjonn", d = ddt, weight = "vekt", total = F)
drikkfreqMen <- proscat_weighted("year", "drukket2rec", d = ddt[kjonn == 1], weight = "vekt", total = F)
drikkfreqWom <- proscat_weighted("year", "drukket2rec", d = ddt[kjonn == 2], weight = "vekt", total = F)
...
drk6 <- proscat_weighted(x = "year", y = "drink6", d = ddt, weight = "vekt", total = FALSE)
drk6kjonn <- proscat_weighted(x = "year", y = "drink6", z = "kjonn", d = ddt, weight = "vekt", total = FALSE)
...
drk6Men <- proscat_weighted("year", "drink6", d = ddt[kjonn == 1], total = F, weight = "vekt", z = "agecat")
drk6Wom <- proscat_weighted("year", "drink6", d = ddt[kjonn == 2], total = F, weight = "vekt", z = "agecat")
```
...
source(file.path(curdir, "analysis2012_2025/drikke-enhet-1224.R"))

# Fit model by year and get adjusted mean for each year
enhetTbl <- ddt[, {
  model <- lm(totalcl ~ alder + kjonn, weights = vekt, data = .SD)
  em <- emmeans(model, ~ 1)
  em_smry <- summary(em)
  data.table(
...
## By gender 
enhetKjonn <- ddt[, {
  model <- lm(totalcl ~ alder, weights = vekt, data = .SD)
  em <- emmeans(model, ~ 1)
  em_smry <- summary(em)
  data.table(
    adj_mean = em_smry$emmean,
